"""
Optimized LBVH v3 - Custom Radix Sort integration.

Key improvement over v2:
- Uses custom GPU Radix Sort instead of Taichi's parallel_sort
- Eliminates the 40-50% sorting overhead identified in benchmarks

Based on benchmark findings:
- parallel_sort was the main bottleneck for large N
- Custom radix sort targets this specific bottleneck
"""

import taichi as ti
import numpy as np
from .radix_sort_gpu import RadixSortGPU_Optimized


@ti.data_oriented
class LBVH_Optimized_V3:
    """
    LBVH v3 with custom Radix Sort.

    Optimizations:
    - Custom 8-bit radix sort (8 passes for 64-bit Morton codes)
    ...
    def __init__(self, max_primitives: int):
        ...
        # Custom radix sorter
        self.radix_sorter = RadixSortGPU_Optimized(max_primitives)
...
@ti.data_oriented
class LBVH_Triangles_V3(LBVH_Optimized_V3):
    """Optimized LBVH v3 for triangles with custom Radix Sort."""
    ...
    def reduce_scene_bounds(self, n_triangles: ti.i32):
        ...
        # Serial reduction of block bounds
        for bid in range(n_blocks):
    ...
    def sort_morton_codes(self, n_triangles: int):
        """Sort using custom GPU Radix Sort."""
        self.radix_sorter.sort(self.morton_codes, self.sorted_indices, n_triangles)
    ...
    def build(self, vertices, triangles, n_triangles: int):
        """Build BVH with custom radix sort."""
        if n_triangles < 1:
            return
...
@ti.data_oriented
class LBVH_Edges_V3(LBVH_Optimized_V3):
    """Optimized LBVH v3 for edges with custom Radix Sort."""
    ...
    def sort_morton_codes(self, n_edges: int):
        """Sort using custom GPU Radix Sort."""
        self.radix_sorter.sort(self.morton_codes, self.sorted_indices, n_edges)
