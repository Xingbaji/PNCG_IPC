"""
Optimized collision detection module using Linear BVH (LBVH) for broad-phase.

Key optimizations over collision_detection_bvh.py:
1. Packed AABB storage - reduces memory loads during traversal
2. Two-pass contact enumeration - reduces atomic contention
3. Distance-based child ordering - visits closer child first for better pruning
4. Prefetched traversal - loads both children before deciding which to explore
5. Optimized narrow-phase with early exits

Based on Stiff-GIPC collision detection with Taichi optimizations.
"""
...
@ti.data_oriented
class collision_detection_bvh_optimized_module(pncg_base_deformer):
    """
    Optimized collision detection using LBVH for broad-phase.
    Supports Point-Triangle (PT) and Edge-Edge (EE) collision detection.

    Optimizations:
    - Packed AABB traversal reduces memory bandwidth
    - Two-pass contact enumeration avoids atomic contention
    - Distance-based child ordering improves pruning
    - Fused narrow-phase operations
    """
    ...
    def init_bvh(self):
        """Initialize optimized BVH data structures."""
        print('Initializing optimized BVH structures...')

        # Create optimized BVH for triangles (for PT queries)
        self.bvh_triangles = LBVH_Triangles_Optimized(self.n_boundary_triangles)

        # Create optimized BVH for edges (for EE queries)
        self.bvh_edges = LBVH_Edges_Optimized(self.n_boundary_edges)

        # Max number of constraints
        self.MAX_C = 2 ** 21

        # Contact pair struct (same as original for compatibility)
        ...
        # Two-pass enumeration buffers for reduced atomic contention
        # First pass: count contacts per thread block
        self.BLOCK_SIZE = 256
        self.n_pt_blocks = (self.n_boundary_points + self.BLOCK_SIZE - 1) // self.BLOCK_SIZE
        self.n_ee_blocks = (self.n_boundary_edges + self.BLOCK_SIZE - 1) // self.BLOCK_SIZE
        self.block_counts = ti.field(dtype=ti.i32, shape=max(self.n_pt_blocks, self.n_ee_blocks) + 1)
        self.block_offsets = ti.field(dtype=ti.i32, shape=max(self.n_pt_blocks, self.n_ee_blocks) + 1)

        # Detection threshold
        self._detection_dHat = self.dHat
        self.bvh_gap = ti.sqrt(self._detection_dHat)

        # Use optimized attempt functions
        self.attempt_PT = self.attempt_PT_optimized
        self.attempt_EE = self.attempt_EE_optimized

        print('Optimized BVH initialization complete.')
    ...
    def find_constraints_PT_bvh(self):
        """Find Point-Triangle constraints using optimized BVH traversal.

        Optimizations:
        - Packed AABB access reduces memory loads
        - Prefetch both children before deciding traversal order
        - Early exit on first valid contact (for intersection checks)
        """
        ...
        for pi in range(self.n_boundary_points):
            ...
            while stack_ptr > 0:
                ...
                # Get children indices (single load from packed node data)
                node_data = self.bvh_triangles.node_data[node_id]
                L_idx = node_data[1]
                R_idx = node_data[2]

                # Prefetch both children AABBs for better memory access
                L_aabb = self.bvh_triangles.aabb[L_idx]
                R_aabb = self.bvh_triangles.aabb[R_idx]

                # Check overlap with packed AABB
                L_overlap = self._point_aabb_overlap_packed(xp, L_aabb, gap)
                R_overlap = self._point_aabb_overlap_packed(xp, R_aabb, gap)
    ...
    def find_constraints_EE_bvh(self):
        """Find Edge-Edge constraints using optimized BVH traversal.

        Optimizations:
        - Precompute edge AABB once per query
        - Packed AABB comparison
        ...
        for ei in range(n_edges):
            ...
            while stack_ptr > 0:
                ...
                # Check overlap
                L_overlap = self._aabb_overlap_packed(edge_aabb, L_aabb, gap)
                R_overlap = self._aabb_overlap_packed(edge_aabb, R_aabb, gap)
    ...
    @ti.func
    def _point_aabb_overlap_packed(self, point: ti.math.vec3,
                                    aabb: ti.template(), gap: ti.f32) -> bool:
        """Check point-AABB overlap using packed AABB format.

        aabb format: [lower_x, lower_y, lower_z, upper_x, upper_y, upper_z]
        """
    ...
    @ti.func
    def _aabb_overlap_packed(self, aabb1: ti.template(),
                              aabb2: ti.template(), gap: ti.f32) -> bool:
        """Check AABB-AABB overlap using packed format.

        Both AABBs format: [lower_x, lower_y, lower_z, upper_x, upper_y, upper_z]
        """
    ...
    def _add_contact_pair(self, ids: ti.template(), dist: float,
        ...
        """Add a contact pair to compact storage with atomic counter."""
        idx = ti.atomic_add(self.n_contacts[None], 1)
        if idx < self.MAX_C:
            self.contact_pairs[idx] = self.pair(ids, dist, cord, t_vec)

    @ti.func
    def attempt_PT_optimized(self, triangle_id, p, t0, t1, t2, xp, x0, x1, x2):
        """Optimized Point-Triangle narrow phase.

        Optimizations:
        - Early adjacency check before broadphase
        - Fused distance and barycentric computation
        """
        # Early adjacency check
    ...
    @ti.func
    def attempt_EE_optimized(self, edge_id_0, edge_id_1, a0, a1, b0, b1,
                              x_a0, x_a1, x_b0, x_b1):
        """Optimized Edge-Edge narrow phase.
    ...
    def find_cnts(self, PRINT=False, TIME_LOG=False, use_refit=False):
        ...
        if TIME_LOG:
            ...
            print(f"[BVH-Opt Time] {bvh_op}: {(t_bvh_end - t_bvh_start)*1000:.2f}ms, "
    ...
    def _edge_triangle_intersection_impl(self, count_mode: ti.template()) -> ti.i32:
        """Unified edge-triangle intersection detection using optimized BVH traversal."""
        INVALID = ti.u32(0xFFFFFFFF)
        result = 0
        gap = 0.0
        ...
        for ti_idx in range(self.n_boundary_triangles):
            ...
            if should_process:
                ...
                while stack_ptr > 0:
                    ...
                    # Check left child
                    if self._aabb_overlap_packed(tri_aabb, L_aabb, gap):
                        L_element = self.bvh_edges.node_data[L_idx][3]
                        if L_element != INVALID:
                            if self._check_edge_tri_leaf(L_element, t0, t1, t2, x0, x1, x2):
                    ...
                    # Check right child
                    if self._aabb_overlap_packed(tri_aabb, R_aabb, gap):
                        R_element = self.bvh_edges.node_data[R_idx][3]
                        if R_element != INVALID:
                            if self._check_edge_tri_leaf(R_element, t0, t1, t2, x0, x1, x2):
                                ti.atomic_add(result, 1)
                        else:
                            if stack_ptr < 63:
                                stack[stack_ptr] = R_idx
