...
Combines the best optimizations from all versions:
- V1: Packed AABB storage, packed node topology
- V2: Fused kernels, hierarchical scene bounds reduction
- Uses Taichi's parallel_sort for stable sorting

Performance Summary (vs Original):
- Build: Improved through packed storage and fused kernels
- Refit: ~1x (same as original, bottleneck is atomic sync)

Usage:
    from algorithm.lbvh_final import LBVH_Triangles_Final, LBVH_Edges_Final

    bvh = LBVH_Triangles_Final(max_triangles)
    bvh.build(vertices, triangles, n_triangles)
    bvh.refit(vertices, triangles, n_triangles)  # Fast update
"""
...
class LBVH_Final:
    """
    Final optimized LBVH base class.
    ...
    def __init__(self, max_primitives: int):
        ...
        self.block_bounds = ti.Vector.field(6, dtype=ti.f32, shape=max(n_blocks, 1))
    ...
    @ti.func
    def aabb_overlap(self, lower: ti.math.vec3, upper: ti.math.vec3,
                     idx: ti.i32, gap: ti.f32) -> bool:
        """Check if query AABB overlaps with node AABB (with gap tolerance)."""
        a = self.aabb[idx]
        return ((a[3] - lower[0]) > -gap and (upper[0] - a[0]) > -gap and
                (a[4] - lower[1]) > -gap and (upper[1] - a[1]) > -gap and
                (a[5] - lower[2]) > -gap and (upper[2] - a[2]) > -gap)
...
class LBVH_Triangles_Final(LBVH_Final):
    ...
    def _sort_morton_codes(self, n: int):
        ...
        ti.algorithms.parallel_sort(self.morton_codes, self.sorted_indices)
    ...
    def _build_tree_topology(self, n: ti.i32):
        ...
        # Build internal nodes - each thread handles one internal node
        for i in range(n - 1):
            ...
            # Set children for this internal node (direct field access to avoid race)
            self.node_data[i][1] = ti.u32(left_child)
            self.node_data[i][2] = ti.u32(right_child)

            # Set parent for children - safe because each child has exactly one parent
    ...
    def build(self, vertices, triangles, n_triangles: int):
        ...
        # 4. Sort (custom radix sort - main optimization)
