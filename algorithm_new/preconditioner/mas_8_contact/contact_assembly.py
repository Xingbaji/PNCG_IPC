"""
Contact Assembly Utilities for MAS Preconditioner with BANKSIZE=8.
...
Note: This is adapted from mas_preconditioner_8_contact/contact_assembly.py
with precision support added.
"""
...
@ti.func
def compute_contact_subblock(para0, para, t, coeff, float_type: ti.template()):
    """
    ...
    """
    H_ij = ti.Matrix.zero(float_type, 3, 3)
...
@ti.func
def project_contact_spd(H_ij, para0, para, t, float_type: ti.template()):
    """
    ...
    H_spd = ti.Matrix.zero(float_type, 3, 3)
    if t_norm_sq > 1e-12:
        for di in ti.static(range(3)):
            for dj in ti.static(range(3)):
...
# ============================================================================
# Contact Direction Jacobians
# ============================================================================

@ti.func
def compute_dtdx_t(t, cord, float_type: ti.template()):
    """
    ...
    """
    dtdx_t = ti.Vector.zero(float_type, 12)
...
@ti.func
def compute_d_dtdx(d, cord, float_type: ti.template()):
    """
    ...
    """
    result = ti.Vector.zero(float_type, 3)
    for i in ti.static(range(4)):
        for j in ti.static(range(3)):
...
@ti.func
def compute_spd_contact_hessian_3x3(e, curvature, float_type: ti.template()):
    """
    ...
    Returns:
        3x3 PSD matrix
    """
    H = ti.Matrix.zero(float_type, 3, 3)
    e_sqr = e.norm_sqr()
...
@ti.func
def compute_spd_edge_gradient(e, dHat, kappa, use_cubic: ti.template(), float_type: ti.template()):
    """
...
@ti.func
def compute_spd_edge_hessian(e, dHat, kappa, use_cubic: ti.template(), float_type: ti.template()):
    """
    ...
    return compute_spd_contact_hessian_3x3(e, c, float_type)
...
@ti.func
def compute_friction_projection_matrix(n, float_type: ti.template()):
    """
    ...
    """
    P = ti.Matrix.identity(float_type, 3)
    for i in ti.static(range(3)):
        for j in ti.static(range(3)):
            P[i, j] -= n[i] * n[j]
    return P
...
@ti.func
def compute_friction_lambda(contact_force, dx, n, mu, min_dx):
    """
...
@ti.func
def compute_friction_gradient(contact_force, dx, n, mu, min_dx, float_type: ti.template()):
    """
    ...
    P = compute_friction_projection_matrix(n, float_type)
    return lam * (P @ dx)


@ti.func
def compute_friction_hessian(contact_force, dx, n, mu, min_dx, float_type: ti.template()):
    """
...
def compute_spd_contact_friction_hessian(
    e, dx, dHat, kappa, mu, min_dx, use_cubic: ti.template(), float_type: ti.template()
):
    """
    ...
    """
    # Barrier Hessian
    H = compute_spd_edge_hessian(e, dHat, kappa, use_cubic, float_type)
    ...
    if mu > 0.0:
        ...
        if d > 1e-10:
            ...
            f_contact = compute_spd_edge_gradient(e, dHat, kappa, use_cubic, float_type)
            H_friction = compute_friction_hessian(f_contact, dx, n, mu, min_dx, float_type)
            H = H + H_friction
...
def compute_spd_contact_friction_gradient(
    ...
    """
    # Barrier gradient
    g = compute_spd_edge_gradient(e, dHat, kappa, use_cubic, float_type)
    ...
    if mu > 0.0:
        ...
        if d > 1e-10:
            n = e / d
            g_friction = compute_friction_gradient(g, dx, n, mu, min_dx, float_type)
            g = g + g_friction
...
@ti.func
def extend_spd_contact_hessian_12x12(H_3x3, cord, float_type: ti.template()):
    """
    ...
    """
    H_12x12 = ti.Matrix.zero(float_type, 12, 12)
    for i in ti.static(range(4)):
...
@ti.func
def extend_spd_contact_gradient_12(g_3, cord, float_type: ti.template()):
    """
    ...
    Returns:
        12D gradient vector
    """
    g_12 = ti.Vector.zero(float_type, 12)
    for i in ti.static(range(4)):
        for j in ti.static(range(3)):
