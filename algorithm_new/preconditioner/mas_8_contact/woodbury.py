"""
Woodbury Update Module for MAS Preconditioner with BANKSIZE=8 and Contacts.
...
Adapted from mas_preconditioner_8_contact/woodbury.py with precision support.
...
@ti.data_oriented
class WoodburySupport8:
    """
    Woodbury low-rank update support for MAS Preconditioner with BANKSIZE=8.
    ...
    def __init__(self, preconditioner):
        ...
        Args:
            preconditioner: MASPreconditioner8Contact instance
        """
        ...
        self.float_type = preconditioner.float_type
    ...
    def init_woodbury_structures(self):
        ...
        # For BANKSIZE=8: BLOCK_DOF = 24
        self.woodbury_U = ti.field(
            dtype=self.float_type,
            shape=(n_blocks, self.top_k, BLOCK_DOF)
        )
        ...
        # B*U precomputed product (n_blocks, BLOCK_DOF, top_k)
        self.BU = ti.field(
            dtype=self.float_type,
            shape=(n_blocks, BLOCK_DOF, self.top_k)
        )
        ...
        print(f"[Woodbury-8] Initialized for {n_blocks} subdomains (BANKSIZE=8), top_k={self.top_k}")
    ...
    def _get_current_contacts(self, solver):
        ...
        contact_pairs_np = solver.contact_pairs.to_numpy()
        ids_arr = contact_pairs_np['a']
        dist_arr = contact_pairs_np['b']
        cord_arr = contact_pairs_np['c']
        normal_arr = contact_pairs_np['d']
    ...
    def _compute_BU_and_capacitance(self):
        ...
        U is the update matrix with shape (BLOCK_DOF, num_updates).
        ...
        for block_id in range(self.n_blocks):
            ...
            for i in range(num_updates):
                for j in range(num_updates):
                    ...
                    cap_val = self.float_type(1.0) if i == j else self.float_type(0.0)
        ...
        for block_id in range(self.n_blocks):
            ...
            for lane_i in range(BANKSIZE):
                ...
                if idx_i < self.n_verts:
                    z = ti.Vector.zero(self.float_type, 3)
            ...
            r_vec = ti.Vector.zero(self.float_type, TOP_K)
            for k in range(num_updates):
                r_val = self.float_type(0.0)
            ...
            for lane_id in range(BANKSIZE):
                idx = block_id * BANKSIZE + lane_id
                if idx < self.n_verts:
                    correction = ti.Vector.zero(self.float_type, 3)
                    for k in range(num_updates):
                        for di in ti.static(range(3)):
