...
project_dir = os.path.dirname(unittest_dir)
...
def main():
    ...
    # Compute elastic contribution = total - inertia
    elastic_contrib = diag_00[0, 0] - expected_inertia
    print(f"Elastic contribution to (0,0): {elastic_contrib:.4e}")

    # Check if hierarchy is built - coarse level might be the problem
    print(f"\n--- Hierarchy info ---")
    ...
    if solver.mas.actual_levels > 1:
        ...
        for lvl in range(solver.mas.actual_levels):
            ...
            if next_v < 0:
                print(f"  Level {lvl}: vertex {v} -> NONE (no parent)")
                break
