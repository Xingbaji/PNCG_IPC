...
Compares: Original, Optimized V1, Optimized V2
...
def benchmark_single_scale(n_primitives, verbose=True):
    ...
    if verbose:
        print(f"\n--- Triangle BVH ---")
        print(f"Build:  Orig {results['tri_build_orig_ms']:.3f}ms | V1 {results['tri_build_opt_ms']:.3f}ms ({results['tri_build_speedup']:.2f}x) | V2 {results['tri_build_v2_ms']:.3f}ms ({results['tri_build_speedup_v2']:.2f}x)")
        print(f"Refit:  Orig {results['tri_refit_orig_ms']:.3f}ms | V1 {results['tri_refit_opt_ms']:.3f}ms ({results['tri_refit_speedup']:.2f}x) | V2 {results['tri_refit_v2_ms']:.3f}ms ({results['tri_refit_speedup_v2']:.2f}x)")
        print(f"\n--- Edge BVH ---")
        print(f"Build:  Orig {results['edge_build_orig_ms']:.3f}ms | V1 {results['edge_build_opt_ms']:.3f}ms ({results['edge_build_speedup']:.2f}x) | V2 {results['edge_build_v2_ms']:.3f}ms ({results['edge_build_speedup_v2']:.2f}x)")
        print(f"Refit:  Orig {results['edge_refit_orig_ms']:.3f}ms | V1 {results['edge_refit_opt_ms']:.3f}ms ({results['edge_refit_speedup']:.2f}x) | V2 {results['edge_refit_v2_ms']:.3f}ms ({results['edge_refit_speedup_v2']:.2f}x)")
...
def print_summary_table(all_results):
    ...
    print("=" * 130)
    print(f"{'N':>10} | {'Original':>12} | {'V1 Opt':>12} | {'V1 Spdup':>8} | {'V2 Opt':>12} | {'V2 Spdup':>8} | {'Best':>8}")
    print("-" * 130)
    for r in all_results:
        best = "V2" if r['tri_build_v2_ms'] < r['tri_build_opt_ms'] else "V1"
        print(f"{r['n_primitives']:>10,} | {r['tri_build_orig_ms']:>10.3f}ms | {r['tri_build_opt_ms']:>10.3f}ms | {r['tri_build_speedup']:>7.2f}x | {r['tri_build_v2_ms']:>10.3f}ms | {r['tri_build_speedup_v2']:>7.2f}x | {best:>8}")
    ...
    print("-" * 130)
    for r in all_results:
        best = "V2" if r['tri_refit_v2_ms'] < r['tri_refit_opt_ms'] else "V1"
        print(f"{r['n_primitives']:>10,} | {r['tri_refit_orig_ms']:>10.3f}ms | {r['tri_refit_opt_ms']:>10.3f}ms | {r['tri_refit_speedup']:>7.2f}x | {r['tri_refit_v2_ms']:>10.3f}ms | {r['tri_refit_speedup_v2']:>7.2f}x | {best:>8}")
    ...
    print("-" * 130)
    for r in all_results:
        best = "V2" if r['edge_build_v2_ms'] < r['edge_build_opt_ms'] else "V1"
        print(f"{r['n_primitives']:>10,} | {r['edge_build_orig_ms']:>10.3f}ms | {r['edge_build_opt_ms']:>10.3f}ms | {r['edge_build_speedup']:>7.2f}x | {r['edge_build_v2_ms']:>10.3f}ms | {r['edge_build_speedup_v2']:>7.2f}x | {best:>8}")
...
def analyze_results(all_results):
    ...
    print(f"                        V1 Optimized    V2 Optimized")
    print(f"   Triangle BVH Build:    {avg_tri_build_v1:.2f}x           {avg_tri_build_v2:.2f}x")
    print(f"   Triangle BVH Refit:    {avg_tri_refit_v1:.2f}x           {avg_tri_refit_v2:.2f}x")
    print(f"   Edge BVH Build:        {avg_edge_build_v1:.2f}x           {avg_edge_build_v2:.2f}x")
    print(f"   Edge BVH Refit:        {avg_edge_refit_v1:.2f}x           {avg_edge_refit_v2:.2f}x")
    ...
    for r in all_results:
        ...
        print(f"   N={r['n_primitives']:>7,}: Orig {throughput_orig:>8,.0f}/ms, V1 {throughput_v1:>8,.0f}/ms, V2 {throughput_v2:>8,.0f}/ms")

    # Best/worst case for V2
    speedups_v2 = [r['tri_build_speedup_v2'] for r in all_results]
    best_idx = np.argmax(speedups_v2)
    worst_idx = np.argmin(speedups_v2)

    print(f"\n4. Best/Worst Cases (V2 vs Original):")
    print(f"   Best speedup:  {speedups_v2[best_idx]:.2f}x at N={all_results[best_idx]['n_primitives']:,}")
    print(f"   Worst speedup: {speedups_v2[worst_idx]:.2f}x at N={all_results[worst_idx]['n_primitives']:,}")

    # Recommendations
    print(f"\n5. Recommendations:")
    best_build = "V2" if avg_tri_build_v2 > avg_tri_build_v1 else "V1"
    best_refit = "V2" if avg_tri_refit_v2 > avg_tri_refit_v1 else "V1"
    print(f"   Best for Build: {best_build} (V1: {avg_tri_build_v1:.2f}x, V2: {avg_tri_build_v2:.2f}x)")
    print(f"   Best for Refit: {best_refit} (V1: {avg_tri_refit_v1:.2f}x, V2: {avg_tri_refit_v2:.2f}x)")

    # V2 specific improvements
    print(f"\n6. V2 Optimizations Analysis:")
    print(f"   - Fused leaf AABB + block bounds: Reduces kernel launches")
    print(f"   - Hierarchical scene bounds reduction: Less atomic contention")
    print(f"   - Fused init + build topology: Single kernel for node setup")
    print(f"   - Note: parallel_sort remains the main bottleneck (~40-50% of build time)")


def main():
    print("=" * 80)
    print("Comprehensive BVH Benchmark: Original vs Optimized")
    print("=" * 80)
