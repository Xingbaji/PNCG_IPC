...
# ==============================================================================
# MAS hessian_matvec Accuracy Test (vs compute_zHz ground truth)
# ==============================================================================

class TestHessianMatvecAccuracy(unittest.TestCase):
    """
    Test MAS hessian_matvec accuracy against ground truth compute_zHz.

    This verifies that the MAS block matrices' hessian_matvec produces results
    consistent with the exact Hessian-vector product computed via:
    ...
    @classmethod
    def setUpClass(cls):
        """Initialize Taichi and solver using pncg_ipc_deformer."""
        ti.reset()
        ti.init(arch=ti.cuda, default_fp=ti.f32)
        ...
        # Create a solver class that uses MAS-Small
        class TestSolverForHessian(pncg_ipc_deformer):
            def __init__(self):
                super().__init__(demo='cube_freefall_10')
                self.mesh.verts.place({'z': ti.types.vector(3, float)})
                # Initialize MAS-Small
                self.mas_small = MASPreconditionerSmall(self.mesh)

        cls.solver = TestSolverForHessian()
        cls.n_verts = cls.solver.n_verts
        cls.n_cells = cls.solver.n_cells

        print(f"\n[Setup] Mesh: {cls.n_verts} vertices, {cls.n_cells} cells")

        # Buffers for hessian_matvec
        cls.z_buffer = ti.Vector.field(3, dtype=ti.f64, shape=cls.n_verts)
        cls.Hz_buffer = ti.Vector.field(3, dtype=ti.f64, shape=cls.n_verts)

        print(f"[Setup] MAS-Small initialized with {cls.solver.mas_small.level_num} levels")

    def test_hessian_matvec_vs_ground_truth(self):
        """Test MAS hessian_matvec against ground truth compute_zHz."""
        # Set initial velocity (freefall)
        self._set_initial_velocity(-1.0)

        # Compute gradient using solver's method
        self.solver.assign_xn_xhat()
        self.solver.compute_grad_and_diagH()
        if self.solver.ground_barrier == 1:
            self.solver.add_grad_and_diagH_ground_barrier()

        # Build MAS and apply
        self.solver.mas_small.rebuild(self.solver)
        self.solver.mas_small.apply()

        z_np = self.solver.mesh.verts.z.to_numpy()
        print(f"\n  |z| = {np.linalg.norm(z_np):.6e}")

        # Method 1: Ground truth z^T H z using solver's compute_pHp
        # First copy z to p field
        self._copy_z_to_p()
        zHz_gt = self.solver.compute_pHp()
        print(f"  z^T H z (ground truth, compute_pHp) = {zHz_gt:.6e}")

        # Method 2: MAS hessian_matvec
        self._copy_z_to_buffer()
        self.solver.mas_small.hessian_matvec(self.z_buffer, self.Hz_buffer)

        Hz_np = self.Hz_buffer.to_numpy()
        print(f"  |H @ z| (MAS) = {np.linalg.norm(Hz_np):.6e}")

        zHz_mas = self._compute_zTHz_from_buffer()
        print(f"  z^T H z (MAS) = {zHz_mas:.6e}")

        # Comparison
        ratio = zHz_mas / zHz_gt if zHz_gt != 0 else float('inf')
        print(f"  Ratio (MAS / GT) = {ratio:.4f}")

        # MAS is an approximation, so we check for same order of magnitude
        # This is a known limitation of the multilevel structure
        self.assertGreater(ratio, 0.01,
            f"MAS z^T H z should be positive and at least 1% of ground truth")
        self.assertLess(ratio, 100.0,
            f"MAS z^T H z should not be more than 100x ground truth")

        # For informational purposes
        if abs(ratio - 1.0) < 0.1:
            print("  Status: EXCELLENT (<10% error)")
        elif abs(ratio - 1.0) < 0.5:
            print("  Status: GOOD (<50% error)")
        else:
            print("  Status: APPROXIMATE (same order of magnitude)")
            print("  (This is expected - MAS stores block approximation)")

    def test_alpha_computation_comparison(self):
        """Test alpha = g^T z / z^T H z using both methods."""
        # Set initial velocity (freefall)
        self._set_initial_velocity(-1.0)
        ...
        g_np = self.solver.mesh.verts.grad.to_numpy().flatten()
        z_np = self.solver.mesh.verts.z.to_numpy().flatten()

        gTz = np.dot(g_np, z_np)
        print(f"\n  g^T z = {gTz:.6e}")

        # Ground truth zHz using compute_pHp
        self._copy_z_to_p()
        zHz_gt = self.solver.compute_pHp()
        alpha_gt = gTz / zHz_gt if zHz_gt != 0 else 0
        print(f"  alpha (ground truth H) = {alpha_gt:.6f}")

        # MAS zHz
        self._copy_z_to_buffer()
        self.solver.mas_small.hessian_matvec(self.z_buffer, self.Hz_buffer)
        zHz_mas = self._compute_zTHz_from_buffer()
        alpha_mas = gTz / zHz_mas if zHz_mas != 0 else 0
        print(f"  alpha (MAS H) = {alpha_mas:.6f}")

        # Step size comparison
        print(f"  |alpha * z| (GT):  {abs(alpha_gt) * np.linalg.norm(z_np):.6e}")
        print(f"  |alpha * z| (MAS): {abs(alpha_mas) * np.linalg.norm(z_np):.6e}")

        # Both alphas should be positive for descent direction
        self.assertGreater(alpha_gt, 0, "Ground truth alpha should be positive")
        # MAS alpha might be different but should give a valid step
        if alpha_mas > 0:
            print("  Both alphas are positive - valid descent")
        else:
            print("  Warning: MAS alpha is non-positive")

    @ti.kernel
    def _set_initial_velocity(self, vy: float):
        """Set initial velocity for all vertices."""
        for vert in self.solver.mesh.verts:
            vert.v = ti.Vector([0.0, vy, 0.0])

    @ti.kernel
    def _copy_z_to_p(self):
    ...
    def _copy_z_to_buffer(self):
        """Copy mesh.verts.z to z_buffer."""
        for vert in self.solver.mesh.verts:
            self.z_buffer[vert.id] = ti.cast(vert.z, ti.f64)

    @ti.kernel
    def _compute_zTHz_from_buffer(self) -> float:
        """Compute z^T * (H @ z) from buffers."""
        ret = 0.0
        for i in range(self.n_verts):
            z_i = self.z_buffer[i]
            Hz_i = self.Hz_buffer[i]
            ret += z_i[0] * Hz_i[0] + z_i[1] * Hz_i[1] + z_i[2] * Hz_i[2]
        return ret
...
def main():
    ...
    parser.add_argument('--hessian-matvec', action='store_true', help='Run hessian_matvec test only')
    ...
    if args.hessian_matvec:
        # Run only hessian_matvec tests
        ti.reset()
        ti.init(arch=ti.cuda, default_fp=ti.f32)
        loader = unittest.TestLoader()
        suite = loader.loadTestsFromTestCase(TestHessianMatvecAccuracy)
