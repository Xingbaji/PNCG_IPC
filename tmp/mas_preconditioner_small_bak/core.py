...
class MASPreconditionerSmall:
    ...
    def init_optimized_assembly(self, cells_np: np.ndarray):
        ...
        if self.optimized_cell_data is not None:
            ...
            self.sorted_B = ti.Matrix.field(3, 3, dtype=ti.f32, shape=self.n_cells)
            self.sorted_W = ti.field(dtype=ti.f32, shape=self.n_cells)
    ...
    def _extract_and_sort_cell_data(self, cell_order: np.ndarray, cells_np: np.ndarray):
        ...
        B_temp = ti.Matrix.field(3, 3, dtype=ti.f32, shape=self.n_cells)
        W_temp = ti.field(dtype=ti.f32, shape=self.n_cells)
