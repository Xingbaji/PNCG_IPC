...
# ===================== Cramer's Rule Based Intersection Detection =====================
...
def segment_triangle_intersect_cramer(ve0, ve1, vt0, vt1, vt2):
    ...
    # If both are on the same side, no intersection possible
    d0 = n.dot(ve0 - vt0)
    d1 = n.dot(ve1 - vt0)
    if d0 * d1 > 0.0:
        result = 0
        ...
        if ti.abs(det) < 1e-20:
            ...
            # uvt[0] = u (barycentric coord for vt1)
            # uvt[1] = v (barycentric coord for vt2)
            # uvt[2] = t (segment parameter)
...
@ti.func
def check_ground_penetration(vertex_pos, ground_normal, ground_offset):
    """
    Check if a vertex has penetrated the ground plane.

    Args:
        vertex_pos: Vertex position (vec3)
        ground_normal: Ground plane normal (vec3), pointing outward
        ground_offset: Ground plane offset (scalar), plane equation: n · x = offset

    Returns:
        1 if penetrated, 0 otherwise
    """
    dist = vertex_pos.dot(ground_normal) - ground_offset
    return ti.cast(dist < 0.0, ti.i32)
