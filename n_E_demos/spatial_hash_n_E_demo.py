"""
Spatial Hash n_E Demo - Uses spatial hashing collision detection.

This version uses spatial hashing (Bresenham's line algorithm) for
collision detection instead of BVH. Uses logarithmic barrier functions.

Usage:
    python spatial_hash_n_E_demo.py                     # Interactive mode
    python spatial_hash_n_E_demo.py --headless --frames 50  # Headless with images
    python spatial_hash_n_E_demo.py --fast --frames 50      # Fast mode (no rendering)
    python spatial_hash_n_E_demo.py --profile --frames 100  # Profile mode with detailed timing
"""
...
VERSION_NAME = "spatial_hash"
COLLISION_METHOD = "spatial_hash"
BARRIER_TYPE = "log"


@ti.data_oriented
class pncg_index_spatial_hash(pncg_ipc_deformer):
    """
    Spatial hash version of per-object optimization solver.
    Uses spatial hashing with Bresenham's line algorithm for collision detection.
    """
...
class SpatialHashNEDemoRunner:
    """Demo runner for spatial hash n_E demo using DemoRunner framework."""

    def __init__(self, demo='eight_E_drop_demo_contact'):
        from demo_runner import DemoRunner
        self.solver = pncg_index_spatial_hash(demo=demo)
        self.solver.set_index()
        self.demo_name = f"n_E-spatial_hash ({demo})"
...
def run_profile_mode(frames=100, demo='eight_E_drop_demo_contact'):
    ...
    print(f"dHat: {solver.dHat} kappa: {solver.kappa} barrier_type: {BARRIER_TYPE}")
    print(f"  Vertices: {solver.n_verts}")
    print(f"  Cells: {solver.n_cells}")
    print(f"  Objects: {solver.N_object}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Spatial Hash n_E Demo')
    ...
    if args.profile:
        ...
        runner = SpatialHashNEDemoRunner(demo=args.demo)
        runner.run()
