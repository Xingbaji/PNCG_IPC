...
Usage:
    python test_mas_freefall.py                      # Test with cube_10
    python test_mas_freefall.py --demo cube_20       # Test with cube_20
    python test_mas_freefall.py --demo cube_40       # Test with cube_40
...
@ti.data_oriented
class FreeFallMASValidator(base_deformer):
    """
    ...
    def __init__(self, demo='cube_freefall_10', inversion_method='oneway_gj'):
        """
        Args:
            demo: Demo configuration name (cube_freefall_10, cube_freefall_20, etc.)
        ...
        # Additional fields for PNCG solver
        ...
        self.mesh.verts.place({'x_init': ti.types.vector(3, float)})
        self.mesh.verts.x_init.copy_from(self.mesh.verts.x)
        self.mesh.verts.x_prev.copy_from(self.mesh.verts.x)

        # Solver parameters
        self.epsilon = self.dict.get('epsilon', 1e-7)
        self.iter_max = self.dict.get('iter_max', 100)
    ...
    def compute_grad_and_diagH(self):
        ...
        for c in self.mesh.cells:
            ...
            # Compute elastic gradient and Hessian contribution
            dPsidx, diag_d2Psidx2 = self.compute_dPsidx(F, B, self.mu, self.la)
            _, diag_H = self.compute_diag_d2Psidx2(F, B, self.mu, self.la)

            # Add to vertex gradients and diagonal Hessian
            for i in ti.static(range(4)):
                c.verts[i].grad += W * dPsidx[i]
                c.verts[i].diagH += W * diag_H[i]
    ...
    def step(self, verbose=False, grad_tol=1e-5):
        ...
        # Build MAS hierarchy and matrices at start of frame
        ...
        for iter in range(self.iter_max):
            ...
            if self.check_z_has_nan():
                print(f'  [Warning] MAS produced NaN at iter {iter}')
                break
    ...
    def validate_frame(self, verbose=True):
        ...
        pos_rel_error = pos_error / (np.linalg.norm(gt_pos - self.initial_centroid) + 1e-10)
        ...
        if verbose:
            ...
            print(f"  Centroid: sim={sim_centroid[1]:.6f}, gt={gt_pos[1]:.6f}, err={pos_error:.2e}")
            print(f"  Velocity: sim={sim_velocity[1]:.6f}, gt={gt_vel[1]:.6f}, err={vel_error:.2e}")

        self.frame_results.append(result)
        return result


def add_freefall_demos():
    """Add free-fall demo configurations to model_loading."""
    # These will be added dynamically when running the test
    pass
...
def run_freefall_test(demo='cube_freefall_10', frames=5, initial_vy=-1.0,
    ...
    """
    print(f"\n{'='*70}")
    print(f"MAS Free-Fall Validation Test")
    print(f"{'='*70}")
    print(f"Demo: {demo}")
    ...
    # Create solver
    solver = FreeFallMASValidator(demo=demo, inversion_method=inversion_method)
    solver.iter_max = iter_max

    print(f"Mesh: {solver.n_verts} vertices, {solver.n_cells} cells")
    ...
    for f in range(frames):
        iters, elapsed = solver.step(verbose=False, grad_tol=grad_tol)
    ...
    final_result = solver.frame_results[-1]
    ...
    if passed:
        print(f"\n✓ TEST PASSED: MAS solver produces correct update direction")
    else:
        print(f"\n✗ TEST FAILED: Errors exceed threshold")
